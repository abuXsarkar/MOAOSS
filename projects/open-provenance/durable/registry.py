...
from lib import (PAYLOAD_BITS, bits_to_hex, embed_watermark, extract_watermark,
                 hamming, hex_to_bits, phash_bits)
...
def register(image, manifest_id, registry_path=DEFAULT_REGISTRY, watermark_out=None):
    ...
    reg["entries"].append({
        ...
        "phash": bits_to_hex(phash_bits(img)),
...
def recover(image, registry_path=DEFAULT_REGISTRY, threshold=DEFAULT_THRESHOLD):
    ...
    return {
        "match": mid if confident else None,
        "best_manifest_id": mid,
        "distance": dist,
...
def _main(argv=None):
    ...
    if a.cmd == "register":
        ...
        if res["match"]:
            print(f"RECOVERED: {res['match']}  (distance {res['distance']}/64, "
                  f"next-best {res['next_best_distance']}/64)")
        else:
            print(f"NO CONFIDENT MATCH (best {res.get('best_manifest_id')} at "
                  f"distance {res.get('distance')}/64 > threshold {res.get('threshold')})")
    return 0
